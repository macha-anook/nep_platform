...
const PROMPT_VERSION = "draft-sections-v1";
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (job.job_type !== "draft_sections") throw new Error(`Unsupported job_type: ${job.job_type}`);
    ...
    await adminClient.from("ai_jobs").update({
      ...
      model: ANTHROPIC_MODEL, prompt_version: PROMPT_VERSION,
