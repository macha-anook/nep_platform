...
function normalizePaper(row) {
  return {
    ...
    status: row.paper_status || 'in_progress',
    notes: row.notes || '',
    history: row.history || [],
    ...
    parentPaperId: row.parent_paper_id || null,
    docxPath: row.docx_path || null,
  };
}
...
export const adapter = {
  ...
  // ── PAPERS (versioned manuscripts, generated + user-edited) ──────────────
  ...
  async listPapers(projectId) {
    const { data, error } = await supabase
      ...
      .not('title', 'is', null)
      .order('created_at', { ascending: false });
    check(data, error, 'listPapers');
    return (data || []).map(normalizePaper);
  },

  // Finalizes a startGeneration() job row into a versioned paper. Reuses an
  // existing non-published draft for the same compound (matches the dedupe
  // behaviour the UI already had client-side) instead of always minting a
  // new version.
  async saveGeneratedPaper(jobId, { blob, fileName, title, compound, notes }) {
    const orgId = await getOrgId();
    const { data: job, error: jobErr } = await supabase
      .from('generated_papers').select('id, project_id, history').eq('id', jobId).single();
    check(job, jobErr, 'saveGeneratedPaper:job');

    const { data: draft } = await supabase
      .from('generated_papers')
      .select('id, version, history')
      .eq('project_id', job.project_id)
      .eq('compound_name', compound || '')
      .eq('paper_status', 'in_progress')
      .not('title', 'is', null)
      .neq('id', jobId)
      .limit(1)
      .maybeSingle();

    const targetId = draft?.id || job.id;
    const history = [...(draft?.history || job.history || []), { date: Date.now(), action: 'Generated', fileName }];
    const ext = (fileName || '').split('.').pop() || 'docx';
    const path = `${orgId}/${targetId}.${ext}`;

    const { error: upErr } = await supabase.storage
      .from('papers').upload(path, blob, { upsert: true, contentType: blob.type || 'application/msword' });
    if (upErr) throw new Error(`[saveGeneratedPaper] ${upErr.message}`);

    const { data, error } = await supabase
      .from('generated_papers')
      .update({
        title, compound_name: compound || '', paper_status: 'in_progress',
        version: draft?.version || '1.0', notes: notes || '', history,
        docx_path: path, file_name: fileName, file_size: blob.size,
        file_type: blob.type || 'application/msword', status: 'done',
      })
      .eq('id', targetId)
      .select().single();
    check(data, error, 'saveGeneratedPaper');
    return normalizePaper(data);
  },

  async updatePaperMeta(paperId, { title, notes }) {
    const { data, error } = await supabase
      .from('generated_papers').update({ title, notes }).eq('id', paperId).select().single();
    check(data, error, 'updatePaperMeta');
    return normalizePaper(data);
  },
  ...
  async publishPaper(paperId) {
    const { data, error } = await supabase
      .from('generated_papers')
      .update({ paper_status: 'published', published_at: new Date().toISOString() })
      .eq('id', paperId).select().single();
    check(data, error, 'publishPaper');
    return normalizePaper(data);
  },

  async createPaperRevision(paperId) {
    const { data: parent, error: parentErr } = await supabase
      .from('generated_papers').select('*').eq('id', paperId).single();
    check(parent, parentErr, 'createPaperRevision:parent');

    const { data, error } = await supabase
      .from('generated_papers')
      .insert({
        project_id: parent.project_id, org_id: parent.org_id, generated_by: parent.generated_by,
        status: 'done', title: `${parent.title} (Revision)`, compound_name: parent.compound_name,
        paper_status: 'in_progress', version: (Number(parent.version) + 0.1).toFixed(1),
        published_at: null, parent_paper_id: parent.id, notes: parent.notes,
        history: parent.history || [], docx_path: parent.docx_path,
        file_name: parent.file_name, file_size: parent.file_size, file_type: parent.file_type,
      })
      .select().single();
    check(data, error, 'createPaperRevision');
    return normalizePaper(data);
  },

  async deletePaper(paperId) {
    const { data: row } = await supabase
      .from('generated_papers').select('docx_path').eq('id', paperId).single();
    if (row?.docx_path) {
      await supabase.storage.from('papers').remove([row.docx_path]).catch(() => {});
    }
    const { error } = await supabase.from('generated_papers').delete().eq('id', paperId);
  ...
  async uploadPaperFile(paperId, file) {
    ...
    const { data: paper, error: paperErr } = await supabase
      .from('generated_papers').select('history').eq('id', paperId).single();
    check(paper, paperErr, 'uploadPaperFile:paper');

    const ext = (file.name || '').split('.').pop() || 'docx';
    const path = `${orgId}/${paperId}.${ext}`;
    const { error: upErr } = await supabase.storage
      .from('papers').upload(path, file, { upsert: true, contentType: file.type || 'application/msword' });
    if (upErr) throw new Error(`[uploadPaperFile] ${upErr.message}`);

    const history = [...(paper.history || []), { date: Date.now(), action: 'Uploaded edited version', fileName: file.name }];
    const { data, error } = await supabase
      .from('generated_papers')
      .update({ docx_path: path, file_name: file.name, file_size: file.size, file_type: file.type || 'application/msword', history })
      .eq('id', paperId).select().single();
    check(data, error, 'uploadPaperFile');
    return normalizePaper(data);
  },
  ...
  async getPaperFileUrl(paperId) {
    const { data: row, error: rowErr } = await supabase
      .from('generated_papers').select('docx_path').eq('id', paperId).single();
    check(row, rowErr, 'getPaperFileUrl:row');
    if (!row.docx_path) return null;
    const { data, error } = await supabase.storage
      .from('papers').createSignedUrl(row.docx_path, 3600);
    if (error) throw new Error(`[getPaperFileUrl] ${error.message}`);
