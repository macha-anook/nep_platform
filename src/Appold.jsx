...
const DoctorPagesPanel = ({ study, allPatients, onBack, onPatientsChange }) => {
  ...
  const addNewPatient = (form) => {
    ...
    const newP = {
      ...
      secondaryCompound: form.secondaryCompound?{name:form.secondaryCompound}:null, secondaryDose:form.secondaryDose||"", secondaryDoseUnit:"mg", secondaryFrequency:"",
                      ...
                      <div>
                        ...
                        {p.status==="active"&&(
                          <div style={{display:"flex",gap:8,borderTop:`1px solid ${T.border}`,paddingTop:8,marginTop:8}}>
                            <button onClick={()=>{
                              const wk=(p.weeklyLogs?.length||0)+1;
                              ...
                              borderRadius:4,padding:"4px 10px",cursor:"pointer",fontFamily:"inherit"}}>
                              + Week {(p.weeklyLogs?.length||0)+1} log
                            </button>
                            <button onClick={()=>{
        ...
        <div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
          <Btn style={{padding:"14px 40px",fontSize:15,fontWeight:700}}
            onClick={async(event)=>{
              ...
              try{
                const name=(project?.paper_title||compound?.name||"NEP").replace(/[^a-zA-Z0-9]+/g,"_").slice(0,60)+"_Evidence_Paper.doc";
                const blob=await buildDocxBlob(project,compound,outcomes,refs,(msg)=>{btn.textContent=msg.slice(0,30)+"…";});
                
                // Save blob as base64 for re-download from Papers tab
                const reader=new FileReader();
                reader.onload=()=>{
                  try{
                    const ep=JSON.parse(localStorage.getItem("nep_papers")||"[]");
                    // Check if paper already exists for this compound
                    const existing=ep.findIndex(p=>p.compound===(compound?.name||"")&&p.status!=="published");
                    const paperEntry={
                      id:existing>=0?ep[existing].id:crypto.randomUUID(),
                      title:project?.paper_title||(compound?.name||"")+" Evidence Synthesis",
                      compound:compound?.name||"",version:existing>=0?ep[existing].version:"1.0",
                      status:"in_progress",updatedAt:Date.now(),
                      createdAt:existing>=0?ep[existing].createdAt:Date.now(),
                      fileName:name,fileData:reader.result,fileSize:blob.size,
                      notes:(existing>=0?ep[existing].notes+"\n":"")+"Generated "+new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit"}),
                      team:project?.team||[],affiliation:project?.affiliation||"",journal:project?.journal||"",
                      history:[...(existing>=0?ep[existing].history||[]:[]),
                        {date:Date.now(),action:"Generated",fileName:name}],
                    };
                    if(existing>=0) ep[existing]=paperEntry; else ep.push(paperEntry);
                    localStorage.setItem("nep_papers",JSON.stringify(ep));
...
export default function App(){
  ...
  const [refs,setRefs]                     = useState(()=>{
    try{
      const s = _STORE.refs["_default"];
      if(s?.length) return s;
      return JSON.parse(localStorage.getItem("nep_refs_backup")||"[]");
    }catch(e){ return []; }
  });
  ...
  // Listen for paper saves from generation panel
  useEffect(()=>{
    const h=()=>{try{setPapers(JSON.parse(localStorage.getItem("nep_papers")||"[]"));}catch(e){}};
    window.addEventListener("nep-papers-updated",h);
    return ()=>window.removeEventListener("nep-papers-updated",h);
  },[]);
              ...
              {showCreateStudy&&(
                <CreateStudyModal compounds={compounds}
                  ...
                  onCreate={(study)=>{
                    ...
                    // Navigate to the new study's compound and show doctor pages
                    if(study.compound?.name) setActiveCompound(study.compound.name);
