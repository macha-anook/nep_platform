// NEP Platform v5.65 — 2026-04-08 — CLEAN_REBUILD
...
const AuthScreen=({onAuth})=>{
  ...
  const canSend    = emailValid && (mode==="signin" || name.trim().length>=2);

  const switchMode=(m)=>{
    setMode(m); setStep("email"); setError("");
    setOtpDigits(["","","","","",""]); setResendCd(0);
  };

  const sendOtp=async()=>{
    if(!canSend){ setError("Please fill in all required fields."); return; }
  ...
  const handleOtpKey=(i,e)=>{
    if(e.key==="Backspace"){
      if(otpDigits[i]){ const d=[...otpDigits]; d[i]=""; setOtpDigits(d); }
  ...
  const handleOtpChange=(i,val)=>{
    ...
    if(!digits) return;
    if(digits.length>=6){
      const d=digits.slice(0,6).split("");
      setOtpDigits(d);
      otpRefs.current[5]?.focus();
      doVerify(digits.slice(0,6));
      return;
    }
    const d=[...otpDigits]; d[i]=digits[0]; setOtpDigits(d);
    if(i<5) otpRefs.current[i+1]?.focus();
    const next=[...d]; next[i]=digits[0];
    if(next.every(x=>x)) doVerify(next.join(""));
  };
        ...
        {step==="email"&&(<>
          ...
          <p style={{fontSize:12,color:T.text3,marginBottom:24}}>
            {mode==="signin"
              ...
              :"Fill in your details — we'll email you a verification code."}
          </p>
            ...
            {mode==="register"&&(<>
              ...
              <div>
                ...
                <div style={{display:"flex",gap:8}}>
                  {[
                    {id:"researcher",icon:"🔬",label:"Researcher",desc:"Evidence platform"},
                    {id:"doctor",    icon:"🩺",label:"Doctor",    desc:"Patient management"},
                  ].map(r=>(
                    <button key={r.id} onClick={()=>setRole(r.id)}
                      style={{flex:1,padding:"12px 8px",borderRadius:8,cursor:"pointer",
                        fontFamily:"inherit",textAlign:"center",
                        background:role===r.id?"rgba(0,210,200,0.15)":T.bg3,
                        color:role===r.id?T.teal:T.text1,
                        border:`2px solid ${role===r.id?T.teal:T.border}`,
                        transition:"all 0.15s"}}>
                      <div style={{fontSize:18,marginBottom:4}}>{r.icon}</div>
                      <div style={{fontSize:13,fontWeight:role===r.id?700:500}}>{r.label}</div>
          ...
          <Btn onClick={sendOtp} disabled={loading||!canSend}
            style={{width:"100%",justifyContent:"center",marginTop:20,padding:"11px",
              fontSize:14,fontWeight:600,opacity:(!canSend&&!loading)?0.5:1}}>
            {loading
          ...
          <div style={{marginTop:16,textAlign:"center",fontSize:12,color:T.text3}}>
            ...
            <button onClick={()=>switchMode(mode==="signin"?"register":"signin")}
        ...
        {step==="otp"&&(<>
          <button onClick={()=>{setStep("email");setError("");setOtpDigits(["","","","","",""]);}}
            style={{background:"none",border:"none",color:T.text3,cursor:"pointer",
              fontSize:12,padding:0,marginBottom:20,display:"flex",alignItems:"center",
          ...
          <p style={{fontSize:12,color:T.text3,marginBottom:4}}>
            We sent a 6-digit code to
          </p>
          <div style={{fontSize:14,fontWeight:600,color:T.teal,marginBottom:24}}>
            {email.trim()}
          </div>

          {mode==="register"&&(
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20,
              padding:"10px 14px",background:T.bg3,borderRadius:8,border:`1px solid ${T.border}`}}>
              <span style={{fontSize:20}}>{role==="doctor"?"🩺":"🔬"}</span>
              <div>
                <div style={{fontSize:13,fontWeight:600,color:T.text0}}>
                  {name.trim()}&nbsp;·&nbsp;
                  <span style={{color:T.teal}}>{role==="doctor"?"Doctor":"Researcher"}</span>
                </div>
                <div style={{fontSize:11,color:T.text3}}>
                  {role==="doctor"?"Patient management access":"Evidence platform access"}
                </div>
              </div>
            </div>
          )}
            ...
            {otpDigits.map((d,i)=>(
              <input key={i} ref={el=>otpRefs.current[i]=el}
                ...
                style={{width:46,height:54,textAlign:"center",fontSize:22,fontWeight:700,
                  fontFamily:T.mono,borderRadius:8,
                  border:`2px solid ${d?T.teal:T.border}`,
                  background:T.bg3,color:T.text0,outline:"none",
                  caretColor:"transparent",transition:"border-color 0.15s"}}/>
            ))}
          </div>

          {error&&<div style={{marginBottom:12,padding:"8px 12px",background:T.redBg,
            borderRadius:6,fontSize:12,color:T.red,textAlign:"center"}}>{error}</div>}

          <Btn onClick={()=>doVerify(otpDigits.join(""))}
            disabled={loading||otpDigits.some(d=>!d)}
            style={{width:"100%",justifyContent:"center",padding:"11px",fontSize:14,fontWeight:600,
              opacity:(otpDigits.some(d=>!d)&&!loading)?0.5:1}}>
            {loading
              ?<span style={{animation:"spin 1s linear infinite",display:"inline-block"}}>↻</span>
              :"Verify & Continue →"}
          </Btn>
...
const SEED_COMPOUNDS = [
  ...
  // ── Secondary compounds ──
  {id:"herb-triphala",name:"Triphala",scientific:"Terminalia chebula blend",category:"Herb",shortCode:"TRI",
   extract_form:"Fruit powder",standardisation:"",dose_range:"250-500mg/day",duration_range:"8-16 weeks",
   notes:"Tridosha-balancing Ayurvedic formula. Co-prescribed with Ashwagandha for digestive support and synergistic adaptogenic effect.",
   refs:[
     {ref_id:"REF-TRI-001",authors:"Peterson CT et al.",year:"2017",title:"Randomised double-blind placebo-controlled trial of Triphala for promotion of herbal synergy",journal:"J Altern Complement Med",volume:"23",issue:"12",pages:"943-953",doi:"10.1089/acm.2017.0083"},
   ]},
  {id:"herb-brahmi",name:"Brahmi",scientific:"Bacopa monnieri",category:"Herb",shortCode:"BRA",
   extract_form:"Leaf extract",standardisation:"20% bacosides",dose_range:"250-500mg/day",duration_range:"12-16 weeks",
   notes:"Cognitive adaptogen. Co-prescribed with Ashwagandha for stress-cognition dual presentation. Bacosides support neuroprotection.",
   refs:[
     {ref_id:"REF-BRA-001",authors:"Pase MP et al.",year:"2012",title:"The cognitive-enhancing effects of Bacopa monnieri: a systematic review of randomized, controlled human clinical trials",journal:"J Altern Complement Med",volume:"18",issue:"7",pages:"647-652",doi:"10.1089/acm.2011.0367"},
   ]},
...
const buildDocxBlob = async (project, compound, outcomes, refs, onProgress) => {
  ...
  const _team = project?.team || project?.research_team || [];
  ...
  const _affil = project?.affiliation || "";
...
const ValidateGeneratePanel = ({ outcomes, refs, compound, project,
  ...
  const blockers = [
    ...
    ...outcomes.length===0?["No outcome rows imported — go to Computed Results and import"]:[] ,
    ...!lead?["Lead researcher not set — add in Study Overview → Research Team"]:[] ,
...
export default function App(){
  ...
  /* Async session restore from Supabase on mount */
  useEffect(()=>{
  ...
  // Auto-reseed if seed version changed
  const SEED_VERSION = "v5.66";
  if(localStorage.getItem("nep_seed_version") !== SEED_VERSION) {
    localStorage.setItem("nep_doctor_patients_user-doctor-nep-science",
      JSON.stringify(SEED_PATIENTS));
    localStorage.setItem("nep_doctor_patients_user-doctor2-nep-science",
      JSON.stringify(SEED_PATIENTS_DR2));
    localStorage.setItem("nep_seed_version", SEED_VERSION);
    console.log("Auto-reseeded to", SEED_VERSION);
  }
  ...
  if(authLoading) return(
    <div style={{minHeight:"100vh",background:T.bg0,display:"flex",alignItems:"center",
      ...
      <span style={{animation:"spin 1s linear infinite",display:"inline-block",
        fontSize:22,color:T.teal}}>↻</span>
    </div>
  );
  if(!user) return <AuthScreen onAuth={u=>{setUser(u);if(u.role==="researcher")loadProjects();}}/>;
  if(user.role==="doctor") return (
    <DoctorApp user={user} onSignOut={async()=>{await API.signOut();setUser(null);setAuthLoading(false);}}/>
  );
  if(user.role==="admin") return (
    <AdminPanel user={user} onSignOut={async()=>{await API.signOut();setUser(null);setAuthLoading(false);}}/>
  );
